...
static void json_profile_add(json_object *j_result, t_datab *datab) {
        mx_add_str_to_js(j_result, "Answer", "The password were changed!");
        mx_add_str_to_js(j_result, "Passwd", (char *)datab->password_db);
}
...
json_object *mx_if_change_passwd(json_object *jobj, sqlite3 *db,
    ...
    datab->password_db = mx_js_to_str(jobj, "Passwd");
    if (mx_is_active(jobj, db, datab)) {
        sprintf(sql, "select ID, PASSWORD from USERS where ID = '%s';",
                datab->id);
        ...
        if (datab->passtrigger == 1) {
            ...
            mx_add_str_to_js(j_result, "Answer", "Wrong password!");
    }
    else
