...
json_object *mx_if_get_profile(json_object *jobj, sqlite3 *db, t_datab *datab) {
    ...
    if (mx_is_active(jobj, db, datab)){
        ...
        mx_js_add(j_result, "Answer", "Profile info!");
        mx_js_add(j_result, "Login", (char *)datab->login_db);
        mx_js_add(j_result, "Email", datab->email);
        mx_js_add(j_result, "Mobile", datab->mobile);
    }
    else
