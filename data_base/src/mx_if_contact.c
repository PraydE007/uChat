...
static void mx_contact_insert(json_object *j_result, sqlite3 *db, char *sql) {
    ...
    sprintf(sql, "insert into CONTACTS (OWNER_id, FOLLOWER_id) " \
        ...
        mx_js_add(j_result, "Answer", MX_CONT);
}
...
json_object *mx_if_contact(json_object *jobj, sqlite3 *db, t_datab *datab) {
    ...
    datab->login_db2 = mx_json_to_str(jobj, "Login2");
    ...
    if (datab->logtrigger == 1 && datab->passtrigger == 1)
        mx_contact_insert(j_result, db, sql);
    else
        mx_js_add(j_result, "Answer", MX_ERRCONT);
