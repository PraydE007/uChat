...
void mx_close_profile(GtkButton *button, gpointer data) {
    ...
    gtk_editable_set_editable(GTK_EDITABLE(gui->e_p_login), TRUE);
    gtk_editable_set_editable(GTK_EDITABLE(gui->e_p_email), TRUE);
    gtk_editable_set_editable(GTK_EDITABLE(gui->e_p_number), TRUE);
}
